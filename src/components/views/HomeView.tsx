...
export const HomeView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-12 animate-fadeIn select-none">
      ...
      {/* 🌟 SLEEK HERO BANNER */}
      <div className="relative rounded-3xl p-5 sm:p-8 overflow-hidden bg-gradient-to-r from-emerald-600/25 via-purple-600/20 to-cyan-600/25 border border-white/20 shadow-2xl backdrop-blur-2xl group">
        ...
        <div className="relative z-10 space-y-3">
          ...
          <div className="flex items-center gap-3 pt-1 sm:pt-2">
            <button
              ...
              className="inline-flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 rounded-full bg-[#1db954] hover:bg-[#1ed760] text-black font-extrabold text-xs sm:text-sm shadow-[0_0_25px_rgba(29,185,84,0.7)] hover:shadow-[0_0_35px_rgba(0,242,254,0.8)] hover:scale-105 transition-all duration-300 cursor-pointer"
            >
      ...
      <section>
        <div className="flex items-center justify-between mb-3.5">
          ...
          <span className="text-xs text-emerald-300 font-mono font-bold bg-[#1db954]/20 px-2.5 py-0.5 rounded-full border border-[#1db954]/40">
            Top 5
          </span>
        </div>

        {top5RecentlyPlayed.length === 0 ? (
          <div className="p-6 rounded-2xl bg-white/[0.04] border border-white/10 text-neutral-400 text-center text-xs sm:text-sm font-medium backdrop-blur-md">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 md:gap-4">
            {top5RecentlyPlayed.map((track) => (
              <GlassCard
                ...
                className="flex flex-col gap-2.5 group p-2.5 sm:p-3.5"
              >
                <div className="relative aspect-square rounded-xl overflow-hidden shadow-lg bg-black/40">
                  ...
                  <button
                    ...
                    className="absolute bottom-2 right-2 sm:bottom-3 sm:right-3 w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-[#1db954] hover:bg-[#1ed760] text-black flex items-center justify-center shadow-[0_0_20px_rgba(29,185,84,0.8)] opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 hover:scale-110 active:scale-95 cursor-pointer"
                  >
                ...
                <div>
                  <h3 className="text-xs sm:text-sm font-bold text-white truncate group-hover:text-emerald-300 transition-colors">
                    {track.title}
                  </h3>
                  <p className="text-[11px] sm:text-xs text-neutral-400 truncate mt-0.5">{track.artist}</p>
                </div>
              </GlassCard>
            ))}
          </div>
      ...
      <section>
        <h2 className="text-lg md:text-xl font-bold text-white mb-3.5 flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
          {MUSIC_CATEGORIES.map((cat) => (
            <div
              ...
              className={`p-3.5 rounded-2xl bg-gradient-to-br ${cat.color} cursor-pointer hover:scale-105 transition-all duration-300 shadow-lg border border-white/20 flex flex-col justify-between h-24 group`}
            >
              <span className="text-[10px] font-mono uppercase font-bold text-white/80">Género</span>
              <h3 className="text-xs sm:text-sm font-black text-white leading-tight group-hover:text-cyan-200 transition-colors">
                {cat.name}
              </h3>
            </div>
          ))}
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-[9px] sm:text-[10px] font-mono px-2 py-0.5 rounded bg-white/10 text-neutral-300 truncate max-w-[85px] sm:max-w-[100px]">
                    ...
                    >
                      <Heart className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isLiked ? 'fill-pink-400' : ''}`} />
                    </button>
                  </div>
                </div>
              </GlassCard>
