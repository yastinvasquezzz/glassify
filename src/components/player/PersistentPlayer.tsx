  ...
  return (
    <>
      <div className="fixed bottom-0 left-0 right-0 z-40 px-2 sm:px-3 pb-2 sm:pb-3 pt-0 pointer-events-none select-none">
        <GlassPanel
          className="w-full max-w-7xl mx-auto p-2 sm:p-3 flex flex-col md:flex-row items-center justify-between gap-2 md:gap-3 pointer-events-auto border-white/20 bg-slate-950/90 backdrop-blur-2xl shadow-[0_-10px_40px_rgba(0,0,0,0.85)] overflow-hidden"
          intensity="heavy"
          glow
        >
          {/* MOBILE STREAMLINED PLAYER BAR (< md) */}
          <div className="flex md:hidden flex-col w-full gap-1.5">
            <div className="flex items-center justify-between gap-2.5 w-full">
              {/* Artwork + Track Title */}
              <div
                onClick={toggleNowPlayingExpanded}
                className="flex items-center gap-2.5 overflow-hidden min-w-0 flex-1 cursor-pointer"
              >
                <img
            ...
            {/* Slim Timeline Bar */}
            <div className="flex items-center gap-2 w-full text-[9px] text-neutral-400 font-mono">
          ...
          <div className="hidden md:flex items-center gap-3 w-full md:w-1/4">
            <div
              onClick={toggleNowPlayingExpanded}
              className="relative group cursor-pointer overflow-hidden rounded-xl w-14 h-14 flex-shrink-0 border border-white/25 shadow-lg"
            >
              <img
                src={currentTrack.coverUrl}
                alt={currentTrack.title}
            ...
            <div className="overflow-hidden min-w-0 flex-1">
              <h4
                onClick={toggleNowPlayingExpanded}
                className="text-sm font-bold text-white truncate cursor-pointer hover:text-emerald-300 transition-colors"
              >
                {currentTrack.title}
              </h4>
              <p className="text-xs text-neutral-400 truncate hover:text-neutral-200 cursor-pointer">
                {currentTrack.artist}
              </p>
            </div>
            ...
            <button
              ...
              className={`p-1.5 rounded-full transition-all active:scale-90 ${
                isLiked ? 'text-pink-400 bg-pink-500/20' : 'text-neutral-400 hover:text-white hover:bg-white/10'
              }`}
            >
            ...
            <button
              ...
              className="p-1.5 text-neutral-400 hover:text-white rounded-full transition-all active:scale-90 hover:bg-white/10"
            >
          ...
          <div className="hidden md:flex flex-col items-center gap-1.5 w-full md:w-2/4">
            ...
            <div className="flex items-center gap-2 w-full text-xs text-neutral-400 font-mono">
              <span>{formatTime(currentTime)}</span>
              <div className="relative flex-1 flex items-center group h-3">
                <input
                  ...
                  value={currentTime}
                  onChange={handleSeekChange}
                  className="w-full h-1.5 bg-white/20 group-hover:h-2.5 rounded-lg appearance-none cursor-pointer accent-[#1db954] transition-all"
                />
              </div>
              <span>{formatTime(duration)}</span>
            </div>
          ...
          <div className="hidden md:flex items-center justify-end gap-2 w-full md:w-1/4">
            ...
            <div className="flex items-center gap-1.5 ml-1">
              ...
              <input
                ...
                className="w-16 h-1.5 bg-white/20 rounded-lg appearance-none cursor-pointer accent-[#1db954]"
              />
            </div>
          </div>
        </GlassPanel>
