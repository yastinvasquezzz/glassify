...
export const Sidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      {isMobileSidebarOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex bg-black/80 backdrop-blur-xl animate-fadeIn">
          <div className="w-80 max-w-[85vw] h-full bg-slate-950/95 border-r border-white/15 shadow-2xl p-4 flex flex-col gap-4 overflow-y-auto z-50">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              ...
              >
                <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-emerald-500 via-cyan-500 to-purple-600 p-0.5 shadow-[0_0_15px_rgba(29,185,84,0.5)]">
                  <div className="w-full h-full bg-slate-950 backdrop-blur-md rounded-[10px] flex items-center justify-center">
                    <Music2 className="w-4 h-4 text-emerald-400" />
                  </div>
                </div>
                <div>
                  <h1 className="text-lg font-black tracking-tight text-white flex items-center gap-1 font-mono">
                    GLASSIFY <Sparkles className="w-3 h-3 text-cyan-400" />
                  </h1>
                  <p className="text-[9px] text-emerald-400 tracking-wider uppercase font-bold">
                    MÚSICA HI-FI
                  </p>
                </div>
              </div>
              ...
              {navItems.map((item) => (
                <button
                  ...
                  className={`flex items-center gap-4 px-4 py-3 rounded-xl font-semibold text-sm transition-all duration-300 ${
                    activeView === item.view
                      ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-400/40 shadow-[0_0_20px_rgba(16,185,129,0.3)]'
            ...
            {/* Playlists Section */}
            <div className="flex flex-col gap-3 mt-2 pt-4 border-t border-white/10 flex-1 overflow-y-auto">
              <div className="flex items-center justify-between px-1">
                <span className="text-xs font-bold text-neutral-400 uppercase tracking-wider flex items-center gap-2">
                  <ListMusic className="w-4 h-4 text-emerald-400" /> Tus Playlists
                </span>
                <button
                  onClick={handleCreatePlaylist}
                  className="p-1.5 text-emerald-400 hover:text-white rounded-lg hover:bg-white/10"
                >
                  <PlusSquare className="w-4 h-4" />
                </button>
              </div>

              {/* Liked Songs Quick Pill */}
              <button
                onClick={() => {
                  setActiveView('library');
                  toggleMobileSidebar();
                }}
                className="flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r from-purple-900/40 to-pink-900/40 border border-pink-500/20 text-left hover:border-pink-500/40 transition-all"
              >
                ...
                <div>
                  ...
                  <p className="text-[10px] text-pink-300">{likedTrackIds.length} guardadas</p>
                </div>
              </button>
              ...
              <div className="flex flex-col gap-1.5 overflow-y-auto pr-1">
                {userPlaylists.map((playlist) => (
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => handleDeletePlaylist(e, playlist.id, playlist.title)}
                      className="p-1.5 text-neutral-500 hover:text-red-400 rounded-lg hover:bg-white/10"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
